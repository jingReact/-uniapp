<template>
	<view class="home">
		<!-- 轮播图 -->
		<view class="swiper_box">
			<swiper class="swiper" :indicator-dots="indicatorDots" :circular="true" :autoplay="autoplay" :interval="interval"
			 :duration="duration" @change="bannerfun">
				<swiper-item v-for="(item,index) in bannerlist" :key="index">
					<image :src="httpImg+item.imgxPath" mode=""></image>
				</swiper-item>
			</swiper>

			<!-- 自定义指示器 -->
			<view class="instruct">
				<block v-for="(item,index) in bannerlist" :key="index">
					<view class="instruct_view" :class="{active:index == num}">
					</view>
				</block>

			</view>
		</view>
		<!-- 早中晚 -->
		<view class="cotent">
			<view class="yongcan">
				<block  v-for="(item,index) in bannerlist" :key="index">
					<image :src="httpImg+item.imgPath" mode="" @click="toMerchant(item.id)"></image>
				</block>
			</view>
		</view>
		<!-- 餐品推荐 -->
		<view class="products">
			<view class="head_box">
				<text>餐品推荐</text><text>新 鲜 食 材</text>
			</view>
			<view class="head_list">
				<block v-for="(item,index) in bannerlist" :key="index">
					<text :class="Tag == index ? 'act' : ''" @click="Taglist(index,item.id)">{{item.desc}}</text>
				</block>
			</view>
			<view class="card_box">
				<!-- -->
				<view class="products_card" v-for="(item,index) in CpDatalist" :key="index"   @click="Todetails(item.id)">
					<view class="cp_img">
						 <easy-loadimage mode="scaleToFill"
						                :scroll-top="scrollTop"
						                :image-src="httpImg+item.image">
										</easy-loadimage>
						<!-- <image v-if="item.image" :src="httpImg+item.image" mode=""></image>
						<image v-else src="../../static/cp_bg.png" mode=""></image> -->
						<view class="chouq" v-if="item.valid == 0">
							<view>售罄</view>
						</view>
					</view>
					<view class="dianjia">
						<image src="../../static/dianjia.png" mode=""></image>
						<text>{{hospitalName}}</text>
					</view>
					<view class="cp_mc">
						{{item.name}}
					</view>
					<!-- 预留便签 -->
					<view class="label_box">
						<block v-for="(item1,index1) in item.foodTags" :key="index1">
							<text>{{item1.name}}</text>
						</block>
					</view>
					<view class="cp_xx">
						<view class="jg">
							¥{{item.price}}
						</view>
						<hx-number-box @change="addGoodsChange" :value="item.number" :rowData="item" :clickTime="animaTime"
						  :show="false" :yans="item.valid == 0 ? true:false"></hx-number-box>
					</view>
				</view>

			</view>
		</view>
		
		<!-- 操作说明 -->
		<view class="instructions">
			<view class="head_box">
				关于云医膳
			</view>
			<view class="box_img">
				<block v-for="(item,index) in bannerlist" :key="index">
					<image :src="httpImg+item.imgTopPath" mode=""></image>
				</block>
				
				<!-- <image src="../../static/home/shuom1.png" mode=""></image>
				<image src="../../static/home/shuom2.png" mode=""></image>
				<image src="../../static/home/shuom3.png" mode=""></image> -->
			</view>
		</view>
		<!-- 购物车 -->
		<view class="foot" @touchmove.stop.prevent="mpClear" :style="{height: footHeight}" v-if="showFoot">
			<view class="zz" @click="hideShoppingCar"></view>
			<view class="btn-box" :style="'bottom:'+navH+'px;'"   v-if="goodsTotalNumber!==0">
				<!-- <view class="btn-box-left" @click="contact">
					<view class="imgBox">
						<image src="../../static/merchants/contact.png" mode=""></image>
					</view>
					<text>联系商家</text>
				</view> -->
				<view class="btn-box-line"></view>
				<view class="btn-box-center" @click="showShoppingCar">
					<view class="cart" :animation="cartAnimationData">
						<view class="tag cartNum" v-if="goodsTotalNumber>0">{{goodsTotalNumber}}</view>
						<image :src="goodsTotalNumber ? '/static/merchants/cart.png' : '/static/merchants/cart2.png'" mode=""></image>
					</view>
					<view class="priceBox">
						<view class="hx-txt-18 hx-color-white" v-if="goodsTotalPrice>0">
							￥{{goodsTotalPrice}}
						</view>
						<view class="hx-txt-10 hx-color-gray">
						
							<text>另需配送费</text><text :class="shippingDees == '0' ? 'pay_ation' : ''">¥{{shippingDees}}</text>
						</view>
					</view>
				</view>
				<view class="btn-box-right">
					<view class="jiesuan" v-if="goodsTotalPrice>0 && goodsTotalPrice >= startingPrice" @click="jiesuan">
						去结算
					</view>
					<view class="pscj hx-txt-10 hx-color-gray" v-else>
						<text v-if="startingPrice>0">差￥{{-(goodsTotalPrice-startingPrice)}}起送</text>
					</view>

				</view>
			</view>
			<view class="cart-box" :style="{display: showCar ? 'flex' : 'none'}">
				<view class="box-container operating-box">
					<view class="operating-box_right">

					</view>
					<view class="operating-box_left clear" @click="clearShoppingCart">
						<i class="hxicon-delete"></i>
						<text>清空购物车</text>
					</view>
				</view>
				<view class=" goods-box">
					<view class="" style="flex: 1;">
						<scroll-view scroll-y="true" class="goods-list-scroll" :scroll-top="carGoodsScrollTop">
							<view class="goods-list">
								<view class="box" v-for="(rowData,i) in shoppCart" :key="rowData.id" v-if="rowData.number>0">

									<view class="m-store-item">
										<view class="m-img">
									
											<image style="width: 100%;height: 100%;" :src="httpImg+rowData.image" mode="scaleToFill"></image>
											<text class="actins" v-if="rowData.spec === 1">新品</text>
											<text v-else-if="rowData.spec === 2">招牌</text>
										</view>
										<view class="m-text">
											<view class="m-title">
												{{rowData.name}}
											</view>
											<view class="m-descripe">
												<block v-for="(item2,index) in rowData.foodTags" :key="index">
													<text>{{item2.name}}</text>
												</block>
												
											</view>
											<view class="m-price-box">
												<view class="symbol">￥</view>
												<view class="m-price">{{rowData.price}}</view>
												<view class="m-old-price" v-if="rowData.oldprice">
													<text>￥{{rowData.oldprice}}</text>
												</view>
											</view>
											<view class="m-distance">
												<view :class="'addEle2_' + rowData.id" class="jumpPosition">
												</view>
												<hx-number-box @change="addGoodsChange" :value="rowData.number" :rowData="rowData" :clickTime="animaTime"
												  :show="true" :yans="false"></hx-number-box>

											</view>
										</view>

									</view>
								</view>
							</view>
						</scroll-view>
					</view>

				</view>
			</view>
		</view>
		
		<!-- 手腕带信息确认 -->
		<view class="shouwd" v-if="showPs">
			<view class="xx_content">
				<view class="xx_head">
					配送地址
				</view>
				<view class="xx_middle">
					<view class="xx_list">
						<view class="xx_left">
							<image src="../../static/user/wmm.png" mode=""></image>
							<text>联系人</text>
						</view>
						<view class="xx_ringt">{{userdata.name}}</view>
					</view>
					<view class="xx_list">
						<view class="xx_left">
							<image src="../../static/user/2.png" mode=""></image>
							<text>手机号</text>
						</view>
						<view class="xx_ringt">{{userdata.phone}}</view>
					</view>
					<view class="xx_list">
						<view class="xx_left">
							<image src="../../static/user/3.png" mode=""></image>
							<text>医院名称</text>
						</view>
						<view class="xx_ringt">{{userdata.hospitalName}}</view>
					</view>
					<view class="xx_list">
						<view class="xx_left">
							<image src="../../static/user/4.png" mode=""></image>
							<text>所属科室</text>
						</view>
						<view class="xx_ringt">{{userdata.area}}</view>
					</view>
					<view class="xx_list">
						<view class="xx_left">
							<image src="../../static/user/5.png" mode=""></image>
							<text>楼栋楼层</text>
						</view>
						<view class="xx_ringt">{{userdata.floor}}</view>
					</view>
					<view class="xx_list">
						<view class="xx_left">
							<image src="../../static/user/6.png" mode=""></image>
							<text>床位</text>
						</view>
						<view class="xx_ringt">{{userdata.bed}}</view>
					</view>
				</view>
				<view class="xx_bottom">
					<view @click="editor('edit')">编辑</view><view @click="confirm">确认</view>
				</view>
			</view>
			
		</view>
		<!-- 用户须知  v-if="notice"-->
		<view class="shouwd" v-if="notice">
			<view class="yhxz">
				<view class="head">
					用户须知
				</view>
				<!-- <view class="wz_ts">
					春节期间(2022年1月29日~2月6日)医院食堂暂停线上点餐服务
				</view> -->
				<view class="wz_ms">
					欢迎使用云医膳! 如果有问题请联系客服电话: {{phone}}
				</view>
				<view class="wz_ms">
					{{zc}}
				</view>
				<view class="wz_ms">
					{{wc}}
				</view>
				<view class="wz_ms">
					{{tc}}
				</view>
				<view class="wz_ms">
					注:支持预点餐服务
				</view>
				<view class="xx_bottom">
					<view @click="checkinof">取消</view><view  @click="checkinof">同意</view>
				</view>
			</view>
		</view>

		<tab-bar class="tabbar" :pagePath="'/pages/home/home'"></tab-bar>
	</view>
</template>

<script>
	import http from '@/common/js/http.js';
	import testData from "@/common/js/testdata.js";
	import hxNumberBox from "@/components/uni-number-box/uni-number-box.vue";
	import easyLoadimage from '@/components/easy-loadimage/easy-loadimage.vue';
	//  加入购物车动画组件
	// import flyInCart from '@/components/flyInCart.vue'
	export default {
		components: {
			hxNumberBox,
			easyLoadimage
			// flyInCart
		},
		data() {
			return {
				scrollTop:0,
				navH:"0",
				phone:"",
				tc:"",
				wc:"",
				zc:"",
				notice:false,
				Tag:"0",
				cbId:"3",
				headlist:['元气早餐','美味午餐','健康晚餐'],
				userdata:null,
				showPs:false,
				showCar:false,
				//配送起步价
				startingPrice: 0,
				storeData:[],
				//当前门店购物车
				shoppCart: [],
				//所有购物车
				shoppingCartAll: [],
				//购物车缓存 Storage 名称
				shoppingCartStorageName: 'shopping_cart',
				//配送费
				shippingDees: 0,
				//购物车商品价格合计
				goodsTotalPrice: 0,
				//购物车商品数量合计
				goodsTotalNumber: 0,
				//显示购物车
				cartAnimationData: {},
				//购物车位置数据
				cartBasketRect: {},
				hospitalName:"",
				showFoot: true,
				//foot 高度
				footHeight: '0',
				num: 0,  //自定义指示器
				dateTime: [],
				CpDatalist: [],
				autoplay: true,
				indicatorDots: false,
				interval: 2000,
				duration: 500,
				bannerlist: [],
				httpImg: ""
			}
		},
		methods: {
			checkinof(){
				this.notice = false;
				// console.log('111')
				if(uni.getStorageSync('usertype') === '5' && uni.getStorageSync('user').hasDefaultAddr === 0){
					this.showPs = true;
					// console.log('222')
				}
			},
			confirm(){
				let obj = {
					address:uni.getStorageSync('user').address,
					addressId:uni.getStorageSync('user').addressId,
					area:uni.getStorageSync('user').area,
					areaCode:uni.getStorageSync('user').areaCode,
					bed:uni.getStorageSync('user').bed,
					floor:uni.getStorageSync('user').floor,
					loginPhone:uni.getStorageSync('user').loginPhone,
					mobile:uni.getStorageSync('user').mobile,
					name:uni.getStorageSync('user').name,
					pid:uni.getStorageSync('user').pid,
					defaul:1
				}
				http.getaddress(obj).then(res =>{
					// console.log(res,"55555")
					this.showPs = false
				})
			},
			Taglist(index,id){
				this.Tag = index
				this.cbId = id
				this.getfoods(this.dateTime[0] + '-' + this.dateTime[1] + '-' + this.dateTime[2],this.cbId);
				this.clearShoppingCart()
			},
			editor(type){
				this.showPs = false;
				uni.navigateTo({
					url: `/pages/address/addressManage?type=${type}&data=${JSON.stringify(this.userdata)}`
				})
			},
			formFirstAddGoods() {
				let that = this
				that.formAddGoodsChange(1, that.currentGoodsData)
				// that.touchOnAddGoods('#ggAddBtn', that.currentGoodsData)
			},
			//清空该门店的购物车
			clearShoppingCart() {
				let that = this;
				that.shoppCart = [];
				that.storeData.shopping_cart = [];
				for (let i in that.shoppingCartAll) {
					if (that.shoppingCartAll[i].store_id == that.storeData.store_id) {
						that.shoppingCartAll[i] = that.storeData
					}
				}
				uni.setStorageSync(that.shoppingCartStorageName, that.shoppingCartAll);
			
				for (let i in that.CpDatalist) {
					that.CpDatalist[i].number = 0;
				}
				
				//购物车商品价格合计
				that.goodsTotalPrice = 0;
				//购物车商品数量合计
				that.goodsTotalNumber = 0;
				that.hideShoppingCar();
			},
			//加入购物车
			formAddGoodsChange(number, goodsData) {
				let that = this;
				if (number >= 1) {
					for (let i in goodsData.form_child) {
						if (goodsData.form_child[i].select == true) {
							that.currentGoodsData.current_form = goodsData.form_child[i]
							break
						}
					}
				}
				that.addGoodsChange(number, that.currentGoodsData)
			
			},
			//新增商品计算价格
			addGoodsChange(number, rowData) {
			
				number = Number(number)
				var that = this;
			
				let shoppCart = [];
				let totalPrice = 0;
				let totalNumber = 0;
				let existedGoods = false;
				//门店第一次添加商品
				let isFirstAddGoods = true;
				//是否为有规格的商品
				let isFormGoods = false
				if (rowData.current_form) {
					isFormGoods = true
				}
				let deleteGoods = null
				let carts = uni.getStorageSync(that.shoppingCartStorageName) || [];
				if (carts.length != 0) {
					isFirstAddGoods = false
					//根据缓存数据 获取购物车中属于本商店的商品
					for (let i in carts) {
						if (carts[i].store_id == that.storeData.store_id) {
							shoppCart = carts[i].shopping_cart ? carts[i].shopping_cart : [];
							break;
						}
					}
					//检查该商品是否为第一次添加，
					for (var i in shoppCart) {
						if (shoppCart[i].id == rowData.id) {
							if (isFormGoods) {
								//相同商品比较规格是否也相同
								if (shoppCart[i].current_form.id == rowData.current_form.id) {
									existedGoods = true;
								}
							} else {
								existedGoods = true;
							}
							if (existedGoods) {
								//在购物车中移除该商品
								if (number <= 0) {
									deleteGoods = shoppCart[i];
									break;
								}
								//非第一次添加，直接修改商品数量，并计算出单品合计
								if (isFormGoods) {
									shoppCart[i].price = rowData.current_form.price
									shoppCart[i].oldprice = rowData.current_form.oldprice
									shoppCart[i].total = number * rowData.current_form.price
								} else {
									shoppCart[i].price = rowData.price
									shoppCart[i].total = number * rowData.price
									shoppCart[i].oldprice = rowData.oldprice
								}
								shoppCart[i].number = rowData.number = number
								break;
							}
						}
					}
				}
				//在购物车中移除该商品
				if (deleteGoods != null) {
					if (carts) {
						//根据缓存数据 获取购物车中属于本商店的商品
						for (let i in carts) {
							if (carts[i].store_id == that.storeData.store_id) {
								var index = shoppCart.indexOf(deleteGoods);
			
								if (index > -1) {
									shoppCart.splice(index, 1);
								}
								carts[i].shopping_cart = shoppCart
								uni.setStorageSync(that.shoppingCartStorageName, carts);
								break;
							}
						}
						setTimeout(() => {
							// this.init();
						}, 100)
					} else {
						that.storeData.shopping_cart = []
						uni.setStorageSync(that.shoppingCartStorageName, that.storeData);
					}
			
					return
				}
			
				//第一次添加
				if (!existedGoods) {
					if (rowData.form) {
						rowData.price = rowData.current_form.price
						rowData.oldprice = rowData.current_form.oldprice
						rowData.total = number * rowData.current_form.price
					} else {
						rowData.total = number * rowData.price
					}
					rowData.number = number;
					shoppCart.push(rowData);
				}
			
				//计算总商品数量和总价
				for (var i in shoppCart) {
					//总价
					totalPrice += shoppCart[i].total
					totalNumber += shoppCart[i].number
				}
				//更改商品列表中的已购买数量
				for (let i in that.CpDatalist) {
					if (that.CpDatalist[i].id == rowData.id) {
						that.CpDatalist[i] = rowData
						break;
					}
				}
			
				that.goodsTotalPrice = totalPrice
				that.goodsTotalNumber = totalNumber
			
				that.shoppCart = shoppCart;
				that.storeData.shopping_cart = shoppCart;
			
				// that.setLabelNumber();
				if (isFirstAddGoods) {
					carts.push(that.storeData)
				}
				if (that.goodsTotalNumber == 0) {
					that.hideShoppingCar();
				}
				//购物车商品数据缓存至本地
				uni.setStorageSync(that.shoppingCartStorageName, carts);
			},
			
			//去结算
			jiesuan() {
				let newArr = JSON.parse(JSON.stringify(this.shoppCart))
				newArr.map(item => {
					item.quantity = item.number
					delete item.number
					delete item.image
					delete item.oldprice
					delete item.type_id
					delete item.total
					delete item.oldprice
					delete item.name
					delete item.descripe
				});
				var filternewArr = newArr.filter(function(person,index,arrs){
				 return person.quantity !== 0;
				})
					let parems = {
						come_from: "mobile",
						restaurant_id: uni.getStorageSync("user").hospilId,
						delivery_fee: this.shippingDees,
						entities: filternewArr,
						day:this.dateTime[0] + '-' + this.dateTime[1] + '-' + this.dateTime[2],
					 	is_prepare:1,
						cbId:this.cbId
					}
					http.getcheckout(parems).then(res =>{
						if(res.code == 9999){
							uni.showToast({
							    title: res.msg,
							    icon: 'none',    //如果要纯文本，不要icon，将值设为'none'
							    duration: 2000     
							}) 
							return
						}
						uni.navigateTo({
							url: "/pages/submit/submit?parems=" + JSON.stringify(res.data)
						})
						
					})
			},
			
			
			
			
			//添加商品
			addCart(id){
				console.log("当前id:"+id)
				this.goodsTotalNumber += 1;
			},
			mpClear(e) {
				// TODO nvue 取消冒泡
				e.stopPropagation()
			},
			//显示购物车
			showShoppingCar() {
				if (this.goodsTotalNumber == 0) {
					return;
				}
				this.footHeight = '100%';
				this.showCar = true;
				this.carGoodsScrollTop = 0;
			},
			//隐藏购物车
			hideShoppingCar() {
				this.footHeight = '0';
				this.showCar = false;
			},
			
			
			//跳转详情
			Todetails(id) {
				let day = this.dateTime[0] + '-' + this.dateTime[1] + '-' + this.dateTime[2]
				uni.navigateTo({
					url: "/pages/details/details?id=" + id+"&cbId="+this.cbId+"&day="+ day,
				})
			},
			//跳转商家页面
			toMerchant(id) {
				uni.navigateTo({
					url: "/pages/merchants/merchants?cbId=" + id
				})
				
			},
			getTimeArray(date) {
				const year = date.getFullYear();
				let month = date.getMonth() + 1;
				let day = date.getDate();
				const hour = date.getHours();
				const minute = date.getMinutes();
				const second = date.getSeconds();
				if (month >= 10) {
					month = month;
				} else {
					month = "0" + month;
				}
				if (day >= 10) {
					day = day;
				} else {
					day = "0" + day;
				}
				return this.dateTime = [year, month, day, hour, minute, second];
			},
			//餐品推荐
			getfoods(day ,id) {
				http.getfoods({
					restaurant_id: uni.getStorageSync("user").hospilId,
					day: day,
					cbId:id
				}).then(res => {
					// console.log(res, "123")
					this.CpDatalist = res.data.records
					// console.log(res, "获取所有商品")
				})
			},
			bannerfun(e) {
				this.num = e.detail.current
				// console.log(this.num)
				
			},
			getfoodCbV() {
				uni.showLoading({
				    title: '加载中...',
					mask:true
				});
				http.getfoodCbV({
					"restaurant_id": uni.getStorageSync("user").hospilId
				}).then(res => {
					this.bannerlist = res.data
					uni.hideLoading();
				})
			}
		},
		onLoad(option) {
			uni.hideTabBar({});
			this.navH = this.$store.state.navHeight + 5;
			if(uni.getStorageSync('usertype') === '5' || uni.getStorageSync('usertype') === '3'){
				this.notice = true
			}
			this.userdata = uni.getStorageSync("user")
			this.storeData = testData.storeData,
			this.hospitalName =  uni.getStorageSync("user").hospitalName
			this.shippingDees = uni.getStorageSync("user").shop.float_delivery_fee
			this.tc = uni.getStorageSync("user").shop.tc
			this.wc = uni.getStorageSync("user").shop.wc
			this.zc = uni.getStorageSync("user").shop.zc
			this.phone = uni.getStorageSync('user').shop.phone
			var myDate = new Date();
			// myDate.setTime(myDate.getTime() + 24 * 60 * 60 * 1000);
			this.getTimeArray(myDate);
			this.getfoods(this.dateTime[0] + '-' + this.dateTime[1] + '-' + this.dateTime[2],this.cbId);
			this.httpImg = testData.httpImg
			// console.log(this.httpImg,"111")
			this.getfoodCbV();
		},
		onPageScroll({scrollTop}) {
		    // 传入scrollTop值并触发所有easy-loadimage组件下的滚动监听事件
		    this.scrollTop = scrollTop;
			// console.log(this.scrollTop,"123")
		},
		onReady() {
			
			const that = this
			let q = uni.createSelectorQuery()
		},
		onShow() {
			
			this.clearShoppingCart()
		}
	}
</script>

<style lang="scss" scoped>
	.hx-color-white {
		color: #FFFFFF;
	}
	//主题颜色
	$hx-theme-color: #FFC107;
	$hx-theme-color-light: #FFEB3B;
	.home {
		background-color: #f6f6f6;

		.swiper_box {
			width: 100%;
			height: 450upx;
			position: relative;

			.swiper {
				width: 100%;
				height: 100%;

				image {
					width: 100%;
					height: 100%;
				}
			}

			.instruct {
				position: absolute;
				bottom: 30upx;
				left: 325upx;
				// transform: translate(0,-50%);
				width: 100upx;
				display: flex;
				justify-content: space-between;
				box-sizing: border-box;
				// padding-right: 60upx;

			}

			.instruct_view {

				width: 20upx;
				height: 6upx;
				border-radius: 6upx;
				margin-left: 10upx;
				background-color: #BEBBBB;
			}

			.active {
				background-color: #000;
			}
		}

		.cotent {
			width: 100%;
			padding: 40upx 20upx 0;
			box-sizing: border-box;

			.yongcan {
				width: 100%;
				height: 280upx;
				display: flex;
				justify-content: space-between;
				flex-wrap: wrap;
				image{
					width: 345upx;
					height: 120upx;
				}
			}
		}

		.products {
			width: 100%;
			padding: 10upx 20upx 0;
			box-sizing: border-box;

			.head_box {
				display: flex;
				justify-content: left;
				margin-bottom: 15upx;
				align-items: center;
				text:first-child{
					font-weight: 700;
					font-size: 34upx;
					height: 50upx;
					line-height: 50upx;
					margin-right: 10upx;
				}
				text:last-child{
					font-size: 20upx;
					background-color:#1DA261;
					color:#fff;
					width: 130upx;
					height: 36upx;
					line-height: 36upx;
					text-align: center;
					border-top-right-radius: 18upx;
					border-bottom-right-radius: 18upx;
					border-top-left-radius: 18upx;
				}
			}
			// .head_box::after{
			// 	position: absolute;
			// 	right: 0;
			// 	top: 0;
			// 	display: block;
			// 	width: 100upx;
			// 	height: 40upx;
			// 	font-size: 22upx;
			// 	content:"新鲜食材";
			
			// },
			.head_list{
				display: flex;
				justify-content: left;
				align-items: center;
				margin-bottom: 20upx;
				text{
					font-size: 26upx;
					width: 140upx;
					height: 50upx;
					margin-right: 10upx;
					text-align: center;
					line-height: 50upx;
					background-color: #fff;
					border-radius: 10upx;
					
				}
				.act{
					background-color: #F9DC67;
				}
			}

			.card_box {
				display: flex;
				flex-wrap: wrap;
				justify-content: space-between;
				width: 100%;
				// margin-bottom: 100upx;

				.products_card {
					width: 345upx;
					padding: 10upx 10upx 30upx;
					margin-bottom: 20upx;
					box-sizing: border-box;
					background-color: #fff;
					border-radius: 10upx;

					.cp_img {
						position: relative;
						width: 100%;
						height: 280upx;
						margin-bottom: 10upx;

						image {
							width: 100%;
							height: 100%;
							border-radius: 10rpx;
						}
						.chouq{
							position: absolute;
							top: 0;
							left: 0;
							width: 100%;
							height: 100%;
							background-color: rgba($color: #000000, $alpha: 0.3);
							view{
								position: absolute;
								top: 50%;
								left: 50%;
								transform: translate(-50%,-50%);
								width: 100upx;
								height: 60upx;
								font-size: 24upx;
								text-align: center;
								line-height: 60upx;
								border: 1upx solid #fff;
								border-radius: 10upx;
								color: #fff;
							}
						}
					}

					.dianjia {
						display: flex;
						justify-content: left;
						align-items: center;

						image {
							width: 30upx;
							height: 30upx;
							border-radius: 15upx;
							margin: 0 10upx;
						}

						text {
							font-size: 20upx;
							color: #B4B4B4;
						}
					}

					.cp_mc {
						font-size: 28upx;
						height: 50upx;
						line-height: 50upx;
						margin-left: 10upx;
						margin-bottom: 20rpx
					}

					.label_box {
						// height: 70rpx;
						width: 100%;
						display: flex;
						justify-content: left;
						margin-bottom: 10upx;

						text {
							font-size: 18upx;
							padding: 5upx;
							border: 1upx solid #EA3C0C;
							margin-left: 10upx;
							border-radius: 5upx;
							color: #EA3C0C;
						}
					}

					.cp_xx {
						width: 100%;
						display: flex;
						justify-content: space-between;
						align-items: center;
						padding: 0 10upx;
						box-sizing: border-box;

						.jg {
							color: #EA3C0C;
							font-size: 26upx;
							font-weight: 700;
						}

						.yd {
							width: 100upx;
							height: 50upx;
							background-color: #EA3C0C;
							font-size: 22upx;
							line-height: 50upx;
							border-radius: 25upx;
							color: #fff;
							text-align: center;
						}
					}
				}


			}

		}
		.instructions{
			width: 100%;
			padding: 10upx 20upx 0;
			box-sizing: border-box;
			margin-bottom: 110upx;
			.head_box {
				font-weight: 700;
				font-size: 34upx;
				margin-bottom: 30upx;
			}
			.box_img{
				image{
					display: block;
					width: 100%;
					height: 360upx;
				}
			}
		}
		.foot {
			position: fixed;
			display: flex;
			z-index: 999;

			justify-content: center;
			align-items: center;
			bottom: 0;
			height: 100%;
			width: 100%;

			/*width: calc(100% - 32px);*/
			.btn-box {
				position: absolute;
				display: flex;
				// bottom: 30upx;
				// bottom: 110upx;
				justify-content: center;
				align-items: center;
				margin: 0;
				height: 100upx;
				width: calc(100% - 64upx);
				z-index: 9;

				&-left {
					background: #222222;
					border-top-left-radius: 100upx;
					border-top-right-radius: 18upx;
					border-bottom-left-radius: 100upx;
					border-bottom-right-radius: 18upx;
					padding-left: 6upx;
					display: flex;
					flex-direction: column;
					justify-content: center;
					align-self: center;
					width: 140upx;
					height: 100%;
					color: #f6d200;
					text-align: center;

					.imgBox {
						display: flex;
						text-align: center;
						justify-content: center;

						image {
							width: 40upx;
							height: 40upx;
						}
					}

					text {
						font-size: 20upx;
					}
				}

				&-line {
					background: #ffffff;
					width: 4upx;
					height: 100%;
				}

				&-center {
					height: 100%;
					flex: auto;
					display: flex;
					justify-content: flex-start;
					align-self: center;
					align-items: center;
					background: #222222;
					border-top-left-radius: 50upx;
					border-bottom-left-radius: 50upx;
					padding-left: 50upx;

					.cart {
						position: relative;
						width: 72upx;
						height: 72upx;
						margin-right: 10upx;
						image {
							width: 100%;
							height: 100%;
						}

						.tag {
							position: absolute;
							right: 12upx;
							top: 16upx;
							height: 36upx;
							width: 36upx;
							background-color: #ff4000;
							color: #ffffff;
							border-radius: 50%;
							z-index: 1;
							font-size: 20upx;
							text-align: center;
							line-height: 36upx;
						}
					}

					.priceBox {
						flex: auto;
					}

				}

				&-right {
					width: 140upx;
					height: 100%;
					position: relative;
					display: flex;
					justify-content: flex-start;
					align-self: center;
					align-items: center;


					.pscj {
						width: 100%;
						height: 100%;
						border-top-right-radius: 100upx;
						border-bottom-right-radius: 100upx;
						background: #222222;
						text-align: center;
						display: flex;
						justify-content: center;
						align-self: center;
						align-items: center;
					}

					.jiesuan {
						width: 100%;
						height: 100%;
						font-size: 28upx;
						border-top-right-radius: 100upx;
						border-bottom-right-radius: 100upx;
						text-align: center;
						display: flex;
						justify-content: center;
						align-self: center;
						align-items: center;
						background: linear-gradient(45deg, $hx-theme-color-light, $hx-theme-color);
						font-weight: bold;
						color: #222222;
					}
				}
			}

			.zz {
				position: absolute;
				top: 0;
				left: 0;
				right: 0;
				bottom: 0;
				background-color: rgba(0, 0, 0, .7);
				z-index: 1;
			}

			.cart-box {
				display: flex;
				justify-content: flex-start;
				flex-flow: column;
				background: #ffffff;
				position: absolute;
				bottom: 0;
				z-index: 2;
				max-height: 66%;

				// padding-bottom: 62px;
				padding-bottom: 224upx;
				border-top-left-radius: 32upx;
				border-top-right-radius: 32upx;
				overflow: hidden;
				width: 100%;

				.rebate-box {
					height: 60rpx;
					background: #FFC107;
					color: #FF9900;
					text-align: center;
					line-height: 60rpx;
					font-size: 28rpx;
				}

				.box-container {
					box-sizing: border-box;
					padding: 0 32upx;
				}

				.operating-box {
					font-size: 24upx;
					line-height: 80upx;
					height: 80upx;
					border-bottom: 2upx solid #f6f6f6;
					color: #666666;
					display: flex;
					flex-direction: row;

					&_right {
						flex: 1;
					}

					&_left {
						display: flex;
						flex-direction: row;
					}

				}

				.goods-box {
					height: 100%;
					overflow: hidden;
					flex: 1;
					display: flex;

					.goods-list-scroll {
						height: 100%;

						.goods-list {

							width: 100%;
							display: flex;
							flex-wrap: wrap;

							.box {
								width: 100%;
								border-bottom: 2upx solid #f6f6f6;
								box-sizing: border-box;
								padding: 0 36upx;

								.m-store-item {
									display: flex;
									flex-direction: row;
									width: 100%;
									justify-content: space-between;
									align-items: flex-end;
									padding-top: 30upx;
									padding-bottom: 30upx;

									.m-img {
										position: relative;
										flex: 0 0 170upx;
										height: 170upx;
										background: #eee;
										border-radius: 8upx;

										text {
											position: absolute;
											top: 0;
											left: 0;
											display: block;
											height: 32upx;
											padding: 0 10upx;
											font-size: 22upx;
											background-color: #f6d200;
											border-bottom-right-radius: 8upx;
											border-top-left-radius: 8upx;
										}

										.actins {
											background-color: #27C56E;
										}
									}

									.m-text {
										flex: 1;
										position: relative;
										height: 170upx;
										padding: 0 12upx;
										display: flex;
										align-content: space-between;
										flex-direction: column;

										.m-title {
											font-size: 28upx;
											color: #555555;
											height: 42upx;
											font-weight: bold;
										}

										.m-descripe {
											font-size: 24upx;
											color: #999999;
											// margin-top: 5px;
											height: 52upx;
											display: flex;
											justify-content: left;
											align-items: center;

											text {
												height: 40upx;
												border-radius: 6upx;
												background-color: #eee;
												font-size: 24upx;
												padding: 0 6upx;
												line-height: 40upx;
												margin-right: 10upx;
											}
										}

										.m-price-box {
											height: 48upx;
											font-weight: bold;
											display: flex;
											flex-direction: row;
											align-items: flex-end;

											.symbol {
												color: #ff582b;
												font-size: 24upx;
											}

											.m-price {
												position: relative;
												top: 4upx;
												font-size: 32upx;
												color: #ff582b;
											}

											.m-old-price {
												margin-left: 6upx;
												display: flex;
												flex-direction: row;
												font-size: 20upx;
												color: #999999;
												margin-top: 5upx;
												text-decoration: line-through;
												font-weight: normal;
											}
										}

										.m-distance {
											position: absolute;
											right: 0;
											bottom: 10upx;
											z-index: 16;
											color: #b2b2b2;
											font-size: 20upx;
											text-align: right;

											.jumpPosition {
												position: absolute;
												bottom: 46upx;
												right: 0;
												z-index: 2;
												width: 56upx;
												height: 56upx;
											}
										}

									}

								}
							}
						}
					}
				}
			}
		}
		.hx-txt-10 {
			font-size: 20upx;
		}
		.hx-color-gray {
			color: #bbbbbb;
			.pay_ation{
				text-decoration:line-through;
			}
		}
	}
	
	.shouwd{
		position: fixed;
		top: 0;
		left: 0;
		right: 0;
		bottom: 0;
		background-color: rgba($color: #000000, $alpha: 0.3);
		.xx_content{
			width: 500upx;
			height: 650upx;
			position: absolute;
			left: 50%;
			top: 50%;
			transform: translate(-50%,-50%);
			background-color: #fff;
			border-radius: 20upx;
			overflow: hidden;
			.xx_head{
				height: 70upx;
				line-height: 70upx;
				background-color: #FAA505;
				text-align: center;
				font-size: 32upx;
				color: #fff;
			}
			.xx_middle{
				width: 100%;
				height: 350upx;
				padding: 20upx 30upx;
				box-sizing: border-box;
				.xx_list{
					display: flex;
					justify-content: space-between;
					align-items: center;
					font-size: 24upx;
					height: 80upx;
					border-bottom: 1upx solid #f6f6f6;
					.xx_left{
						display: flex;
						justify-content: left;
						align-items: center;
						width: 180upx;
						height: 60upx;
						image{
							width: 50upx;
							height: 50upx;
							margin-right: 10upx;
						}
					}
					.xx_ringt{
						color: #ACACAC;
					}
				}
				.xx_list:last-child{
					border: none;
				}
			}
			.xx_bottom{
				position: absolute;
				left: 0;
				bottom: 0;
				height: 70upx;
				width: 100%;
				display: flex;
				justify-content: left;
				view{
					width: 250upx;
					height: 100%;
					line-height: 70upx;
					font-size: 30upx;
					text-align: center;
					box-sizing: border-box;
				}
				view:nth-child(1){
					background-color: #e7e7e7;
				}
				view:nth-child(2){
					color: #fff;
					background-color: #FAA505;
				}
			}
		}
	}

	.yhxz{
		position: absolute;
		left: 50%;
		top: 50%;
		transform: translate(-50%,-50%);
		width: 600upx;
		height: 730upx;
		border-radius: 20upx;
		// background-color: #2C2C2C;
		background-image: url('~@/static/tcbg.png');
		background-size: 100% 100%;
		box-sizing: border-box;
		.head{
			padding: 230upx 0 10upx;
			width: 100%;
			text-align: center;
			color: #D63038;
			font-size: 32upx;
			
		}
		.wz_ms{
			text-align: center;
			width: 100%;
			padding: 0 40upx ;
			box-sizing: border-box;
			line-height: 50upx;
			// color: #868686;
			font-size: 26upx;
		}
		.wz_ts{
			text-align: center;
			width: 100%;
			padding: 0 40upx ;
			box-sizing: border-box;
			line-height: 50upx;
			font-size: 32upx;
			color: red;
			
		}
		.xx_bottom{
				position: absolute;
				left: 0;
				bottom: 0;
				height: 100upx;
				width: 100%;
				display: flex;
				justify-content: space-between;
				
				padding: 0 40upx;
				box-sizing: border-box;
			view{
				width: 240upx;
				height: 70upx;
				line-height: 70upx;
				border-radius: 10upx;
				font-size: 26upx;
				text-align: center;
			}
			view:first-child{
				background-color: #F2F2F2;
			}
			view:last-child{
				background-color: #D63038;
				color: #fff;
			}
		}
	}

</style>
