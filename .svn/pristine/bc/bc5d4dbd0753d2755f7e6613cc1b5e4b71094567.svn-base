<template>
	<view class="uni-load-more" @click="onClick" v-if="status !== 'none'">
		<!-- #ifdef APP-NVUE -->
		...
		<!-- #endif -->
		<text class="uni-load-more__text" :style="{color: color}">{{ status === 'more' ? contentText.contentdown : status === 'loading' ? contentText.contentrefresh : status === 'noMore' ? contentText.contentnomore : '' }}</text>
	</view>
</template>
