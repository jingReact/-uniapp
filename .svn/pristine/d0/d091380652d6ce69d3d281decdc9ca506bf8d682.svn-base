		...
		<view class="hx-comment_basic-info">
			<view class="hx-comment_basic-info_left">
				<view class="hx-comment_basic-info_left_top">
					<text>{{total_overall_rating}}</text>
				</view>
			...
			<view class="hx-comment_basic-info_center">
				<view class="hx-comment_basic-info_center_item" style="margin-bottom: 10px;">
					...
					<view class="hx-comment_basic-info_center_item_center">
						<uni-rate size="14" :value="total_package_rating" :disabled="true"></uni-rate>
					</view>
					<view class="hx-comment_basic-info_center_item_right">
						<text>{{total_package_rating}}</text>
					</view>
				</view>
				<view class="hx-comment_basic-info_center_item">
					...
					<view class="hx-comment_basic-info_center_item_center">
						<uni-rate size="14" :value="total_taste_rating" :disabled="true"></uni-rate>
						
					</view>
					<view class="hx-comment_basic-info_center_item_right">
						<text>{{total_taste_rating}}</text>
					</view>
				</view>
			</view>
		...
		<view class="hx-comment_main-box">
			...
			<view class="lists" v-if="listData.length > 0">
				<block v-for="(item, index_) in listData" :key="index_">
					<view class="item">
						<view class="icon"><image :src="item.image" mode="widthFix" style="width:100%" /></view>
						<view class="info">
							<view class="name-time">
								<text class="name">{{ desen(item.use_id) }}</text>
								<text class="time" v-if=" item.overall_rating == 5">好评</text>
								<text class="time" v-else-if="3 < item.overall_rating <= 4">中评</text>
								<text class="time" v-else-if="3 > item.overall_rating">差评</text>
							</view>
							<view class="stars">
								<text>评分</text> <uni-rate size="14" :value="item.overall_rating" :disabled="true"></uni-rate>
							</view>
							<view class="evaluate-content">
								...
								<view class="imgs" v-if="item.image">
									...
									<!-- @click="previewImgs(imgurl,item.imgs)" -->
										<view class="imgs-box"><image :src="item.image" mode="widthFix" style="width: 100%;" ></image></view>
									<!-- </block> -->
								</view>
							</view>
						</view>
...
<script>
	...
	export default {
		...
		methods: {
			//评价手机号脱敏
			desen(e){
			var desensitization	= e.replace(/(\d{3})\d*(\d{4})/, "$1****$2")
